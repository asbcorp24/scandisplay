...
## 9. Первый запуск и автоматическое создание config.ini
...
При наведении курсора на значок во время записи подсказка показывает, сколько минут и секунд идёт запись, а также текущий размер создаваемого MP4-файла. Данные обновляются каждую секунду.
